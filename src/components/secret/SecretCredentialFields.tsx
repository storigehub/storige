  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        ...
        <button onClick={onAdd} className="text-xs text-pink-accent font-medium">
          + 추가
        </button>
      </div>
        ...
        <div key={idx} className="bg-[#fafafa] rounded-xl p-3 mb-2 space-y-2">
          ...
          {(['service', 'username', 'password', 'memo'] as const).map((field) => (
            <input
              ...
              className="w-full px-3 py-2 border border-[#e0e0e0] rounded-lg text-sm outline-none focus:border-pink-accent font-mono bg-white"
            />
          ))}
        </div>
      ))}
