  ...
  return (
    <div className="px-4 py-3">
      ...
      <div className="grid grid-cols-7 mb-2">
        {DAYS.map((d) => (
          <div key={d} className={`text-center text-xs font-medium py-1 ${d === '일' ? 'text-[#FF4757]' : d === '토' ? 'text-primary' : 'text-[#888]'}`}>
            {d}
          </div>
        ))}
      </div>
            ...
            {day ? (
              <button
                ...
                className={`w-8 h-8 rounded-full flex flex-col items-center justify-center relative transition-colors
                  ${isToday(day) ? 'bg-primary text-white' : 'text-[#1A1A1A]'}
              ...
              >
                ...
                {/* 일기 있음 표시 점 */}
                {entriesSet.has(day) && (
                  <span className={`absolute bottom-0.5 w-1 h-1 rounded-full ${isToday(day) ? 'bg-white' : 'bg-primary'}`} />
                )}
              </button>
            ) : null}
          </div>
