          ...
          {pages.map((_, i) => (
            <button
              ...
              className={`w-1.5 h-1.5 rounded-full transition-colors ${
                i === currentPage ? 'bg-primary' : 'bg-[#e0e0e0]'
              }`}
            />
          ))}
        </div>
...
function PageContent({ page }: { page: BookPage }) {
  if (page.type === 'cover') {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-gradient-to-b from-diary-open to-white px-6 text-center">
  ...
  if (page.type === 'back_cover') {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-primary px-6 text-center">
