...
Step 1: 나의이야기 통합 (3~4주)
    ...
    │   - mystory-khaki.vercel.app → 리다이렉트
    │
    ▼
...
```
1. mystory-khaki.vercel.app에 "Storige로 이전되었습니다" 안내 배너
2. localStorage 데이터 → Storige 계정으로 가져오기 기능 제공
   - 내보내기: JSON 다운로드
   - 가져오기: Storige 앱에서 JSON 업로드 → mystory_sessions에 저장
3. 3개월 후 mystory-khaki.vercel.app → app.storige.co.kr/mystory 리다이렉트
```
...
[리다이렉트]
mystory-khaki.vercel.app  → app.storige.co.kr/mystory  (Step 1 후)
...
- [ ] mystory-khaki.vercel.app 리다이렉트 설정
- [ ] E2E 테스트
